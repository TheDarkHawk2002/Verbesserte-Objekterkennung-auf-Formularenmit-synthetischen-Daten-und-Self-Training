...
\begin{tikzpicture}[scale=0.82]
  ...
  \node[greenaccent] at (2.9,2.25) {\small Annotation};
...
\begin{itemize}
  ...
  \item Der Student lernt aus synthetischen Seiten mit Annotation und realen Seiten mit Pseudo-Labels.
...
\begin{frame}{ASTOD: Adaptive Self-Training for Object Detection}
\begin{center}
...
\begin{tikzpicture}[>=Stealth,node distance=0.45cm,font=\scriptsize,scale=0.97,transform shape]
  \node[draw,rounded corners,fill=softblue,align=center,minimum width=3.8cm,minimum height=0.72cm] (s) {synthetische Seiten + Annotation};
  ...
  \node[draw,rounded corners,fill=softgreen,below=of p,align=center,minimum width=4.6cm,minimum height=0.72cm] (u) {Student: synthetische Seiten und reale Seiten Pseudo-Labeld};
  \node[draw,rounded corners,fill=white,below=of u,align=center,minimum width=4.6cm,minimum height=0.72cm] (f) {Refinement: nur synthetische Annotation};
  \draw[->,thick] (s)--(t);
  \draw[->,thick] (t)--(r);
  \draw[->,thick] (r)--(p);
  \draw[->,thick] (p)--(u);
...
\begin{itemize}
  \item Labeled Data $\rightarrow$ synthetische Seiten mit Annotation
...
\end{center}
\vspace{0.55em}
\takeaway{Direktes Mischen liefert keinen Gewinn; die Einbindung über Self-Training verbessert F1 hier.}
\end{frame}
...
\begin{itemize}
  ...
  \item kein eigener Validierungssatz
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Die synthetische Annotation stabilisiert die wiederholten Trainingsrunden.
