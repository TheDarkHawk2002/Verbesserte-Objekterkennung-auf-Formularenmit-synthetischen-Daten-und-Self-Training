...
\title{Verbesserte Objekterkennung auf Formularen\\mit synthetischen Daten und Self-Training}
\author{Okan Yakin}
...
% 12
\begin{frame}{ASTOD: fünf Schritte einer Runde}
\begin{center}
\begin{tikzpicture}[>=Stealth,node distance=0.34cm,font=\scriptsize]
  \node[draw,rounded corners,fill=white,align=center,minimum width=2.2cm,minimum height=1.05cm] (a) {1. Teacher\\auf Labeled\\Data trainieren};
  \node[draw,rounded corners,fill=white,align=center,right=of a,minimum width=2.2cm,minimum height=1.05cm] (b) {2. Multi-View\\Predictions auf\\realen Seiten};
  \node[draw,rounded corners,fill=white,align=center,right=of b,minimum width=2.2cm,minimum height=1.05cm] (c) {3. NMS und\\Adaptive\\Threshold};
  \node[draw,rounded corners,fill=white,align=center,right=of c,minimum width=2.2cm,minimum height=1.05cm] (d) {4. Student auf\\Labeled +\\Pseudo-Labeled Data};
  \node[draw,rounded corners,fill=softgreen,align=center,right=of d,minimum width=2.2cm,minimum height=1.05cm] (e) {5. Refinement\\nur mit\\Labeled Data};
  \draw[->,thick] (a)--(b);
  \draw[->,thick] (b)--(c);
  \draw[->,thick] (c)--(d);
  \draw[->,thick] (d)--(e);
  \draw[->,thick,greenaccent] (e.south) .. controls +(0,-1.0) and +(0,-1.0) .. node[below,font=\scriptsize]{verfeinerter Student ersetzt den Teacher} (a.south);
\end{tikzpicture}
...
\vspace{0.75em}
\begin{itemize}
  \item Mehrere Ansichten: Original, Spiegelung, Vergrößerung sowie Spiegelung + Vergrößerung.
  \item Predictions werden zurücktransformiert und mit NMS zusammengeführt.
  \item Der Adaptive Threshold wird aus dem Histogramm der Confidence Scores bestimmt.
  \item Unsichere Pseudo-Labels erhalten im Loss ein geringeres Gewicht.
\end{itemize}
\sourcefootnote{ASTOD [4].}
\end{frame}
